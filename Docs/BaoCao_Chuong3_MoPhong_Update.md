...
- **Đánh giá hiệu năng**: Phân tích các chỉ số như dung lượng bí mật ($$ C_s $$), xác suất chặn bí mật (SOP), xác suất chặn (IP), tỷ lệ bí mật ergodic, dung lượng kênh trung bình, và tỷ lệ lỗi bit (BER) trong các điều kiện kênh khác nhau.
...
- **Kết quả phân tích**:
  - **Dung lượng kênh trung bình** ($$ C_{B1}, C_{B2}, C_E $$):
    $$ C_{B1} = B \cdot \mathbb{E}[\log_2(1 + \text{SNR}_{B1})], \quad C_{B2} = B \cdot \mathbb{E}[\log_2(1 + \text{SNR}_{B2})] $$
    $$ C_E = B \cdot \mathbb{E}[\log_2(1 + \max(\text{SNR}_{E1}, \text{SNR}_{E2}))] $$
    Khi $$ \text{SNR}_{\text{Eve}} $$ tăng, $$ C_{B1}, C_{B2} $$ giảm nhẹ do nhiễu từ Eve, trong khi $$ C_E $$ tăng, phản ánh khả năng giải mã của Eve được cải thiện.
  - **Dung lượng bí mật (Secrecy Rate)** ($$ R_{s1}, R_{s2}, R_{s,\text{sum}} $$):
    $$ R_{s1} = \max(0, R_1 - R_{e1}), \quad R_{s2} = \max(0, R_2 - R_{e2}), \quad R_{s,\text{sum}} = R_{s1} + R_{s2} $$
    Với $$ R_1 = \log_2(1 + \text{SNR}_{B1}) $$, $$ R_2 = \log_2(1 + \text{SNR}_{B2}) $$, $$ R_{e1} = \log_2(1 + \text{SNR}_{E1}) $$, $$ R_{e2} = \log_2(1 + \text{SNR}_{E2}) $$. Khi $$ \text{SNR}_{\text{Eve}} $$ tăng, $$ R_{s1}, R_{s2} $$ giảm do $$ R_{e1}, R_{e2} $$ tăng. $$ R_{s1} > R_{s2} $$ vì Bob1 có điều kiện kênh tốt hơn. $$ R_{s,\text{sum}} $$ giảm tuyến tính, cho thấy hệ thống dễ bị tổn thương khi Eve tăng công suất nhiễu.
  - **Tỷ lệ bí mật Ergodic** ($$ Erg_{Rs1}, Erg_{Rs2} $$):
    $$ Erg_{Rs1} = \mathbb{E}[R_{s1}], \quad Erg_{Rs2} = \mathbb{E}[R_{s2}] $$
    Giá trị này tương tự $$ R_{s1}, R_{s2} $$ vì được tính trung bình qua các lần mô phỏng Monte Carlo, phản ánh hiệu năng bảo mật trung bình.
  - **Xác suất chặn bí mật (SOP)**:
...
- **Kết quả phân tích**:
  - **Dung lượng kênh trung bình** ($$ C_{B1}, C_{B2}, C_E $$): $$ C_{B1}, C_{B2} $$ tăng nhẹ khi $$ d_E $$ tăng do nhiễu từ Eve giảm, trong khi $$ C_E $$ giảm do suy hao kênh của Eve tăng.
  ...
  - **Tỷ lệ bí mật Ergodic** ($$ Erg_{Rs1}, Erg_{Rs2} $$): Tương tự $$ R_{s1}, R_{s2} $$, tăng khi $$ d_E $$ tăng, phản ánh hiệu năng bảo mật trung bình cải thiện khi Eve ở xa.
...
### 3.4.1 Thuật Toán Mô Phỏng Kịch Bản 2: Quét $$ \text{SNR}_{\text{Eve}} $$
```plaintext
BẮT ĐẦU
...
3. Vòng lặp qua các giá trị $$ \text{SNR}_{\text{Bob}} $$:
   ...
   - Vòng lặp qua các giá trị $$ \text{SNR}_{\text{Eve}} $$:
     ...
     - Tính tỷ lệ bí mật ergodic: $$ Erg_{Rs1} = \mathbb{E}[R_{s1}] $$, $$ Erg_{Rs2} = \mathbb{E}[R_{s2}] $$.
     - Tính dung lượng kênh trung bình: $$ C_{B1} = B \cdot \mathbb{E}[\log_2(1 + \text{SNR}_{B1})] $$, $$ C_{B2} = B \cdot \mathbb{E}[\log_2(1 + \text{SNR}_{B2})] $$, $$ C_E = B \cdot \mathbb{E}[\log_2(1 + \max(\text{SNR}_{E1}, \text{SNR}_{E2}))] $$.
     ...
     - Tính hiệu suất phổ bí mật: $$ \eta_{s1} = \frac{R_{s1}}{B} $$, $$ \eta_{s2} = \frac{R_{s2}}{B} $$.
     - Lưu kết quả trung bình qua $$ 10^4 $$ lần mô phỏng Monte Carlo.
4. Vẽ biểu đồ: $$ R_{s1}, R_{s2}, R_{s,\text{sum}}, Erg_{Rs1}, Erg_{Rs2}, C_{B1}, C_{B2}, C_E, \text{SOP}, \text{IP}, \text{BER}, \eta_{s1}, \eta_{s2} $$ theo $$ \text{SNR}_{\text{Eve}} $$ tại $$ \text{SNR}_{\text{Bob}} = 20 \, \text{dB} $$.
...
4. Vẽ biểu đồ: $$ R_{s1}, R_{s2}, R_{s,\text{sum}}, Erg_{Rs1}, Erg_{Rs2}, C_{B1}, C_{B2}, C_E, \text{SOP}, \text{IP}, \text{BER}, \eta_{s1}, \eta_{s2} $$ theo $$ d_E $$.
