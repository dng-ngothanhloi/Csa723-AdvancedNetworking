...
# Hàm tính SNR với numba để tối ưu hóa
@jit(nopython=True)
def compute_snr(h_B1, h_B2, h_E, h_EB1, h_EB2, P_A, alpha1, alpha2, N_0, P_E, d_B1, d_B2, d_E, d_EB1, d_EB2, epsilon, N_ant):
    ...
    I_Bob = P_A * norm_h_E / d_E**alpha
    SNR_E1 = (P_A * alpha1 * norm_h_E) / (d_E**alpha * N_0 + I_Bob)
    SNR_E2 = (P_A * alpha2 * norm_h_E) / (d_E**alpha * N_0 + P_A * norm_h_EB2 / d_EB2**alpha)
...
eta_s1 = np.zeros((len(SNR_Bob_dB_range), len(SNR_Eve_dB_range)))  # Hiệu suất phổ bí mật
eta_s2 = np.zeros((len(SNR_Bob_dB_range), len(SNR_Eve_dB_range)))
...
for bob_idx, SNR_Bob_dB in enumerate(SNR_Bob_dB_range):
    ...
    for eve_idx, SNR_Eve_dB in enumerate(SNR_Eve_dB_range):
        ...
        eta_s1[bob_idx, eve_idx] = R_s1[bob_idx, eve_idx] / B
        eta_s2[bob_idx, eve_idx] = R_s2[bob_idx, eve_idx] / B
        ...
        tx_signal = 2 * bits - 1
        rx_signal_B1 = np.sqrt(P_A_eff * alpha1) * np.sum(h_B1, axis=1) * tx_signal + np.sqrt(epsilon * P_A_eff * alpha2) * np.sum(h_B1, axis=1) * tx_signal + np.sqrt(P_E / d_EB1**alpha) * np.sum(h_EB1, axis=1) * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
        rx_signal_B2 = np.sqrt(P_A_eff * alpha2) * np.sum(h_B2, axis=1) * tx_signal + np.sqrt(P_E / d_EB2**alpha) * np.sum(h_EB2, axis=1) * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
        rx_signal_E = np.sqrt(P_A_eff * alpha1) * np.sum(h_E, axis=1) * tx_signal + np.sqrt(P_A_eff * alpha2) * np.sum(h_E, axis=1) * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
...
plt.legend()
plt.grid(True)
plt.savefig('scenario_2_rs1_rs2.png')
plt.show()
plt.close()
...
plt.legend()
plt.grid(True) 
plt.savefig('scenario_2_sop1_sop2.png')
plt.show()
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig('scenario_2_ip1_ip2.png')
plt.show()
plt.close()
...
plt.savefig('scenario_2_ber_b1_b2_e.png')
plt.show()
plt.close()

plt.figure(figsize=(8,6))
plt.plot(SNR_Eve_dB_range, eta_s1[bob_idx_plot], 'g-o', label='Secrecy Spectral Efficiency Bob1')
plt.plot(SNR_Eve_dB_range, eta_s2[bob_idx_plot], 'b-s', label='Secrecy Spectral Efficiency Bob2')
plt.xlabel('SNR_Eve (dB)')
plt.ylabel('Secrecy Spectral Efficiency (bits/s/Hz)')
plt.title('Secrecy Spectral Efficiency vs SNR_Eve (SNR_Bob = 20 dB)')
plt.legend()
plt.grid(True)
plt.savefig('scenario_2_eta_s1_s2.png')
plt.show()
plt.close()
...
SNR_Bob_dB = 20  # SNR_Bob cố định
SNR_Eve_dB = 30  # SNR_Eve cố định
SNR_Bob_linear = 10**(SNR_Bob_dB / 10)
SNR_Eve_linear = 10**(SNR_Eve_dB / 10)
num_samples = int(1e5)  # Tăng số lần mô phỏng
...
eta_s1_3 = np.zeros(len(d_E_range))  # Hiệu suất phổ bí mật
...
for idx, d_E in enumerate(d_E_range):
    ...
    bits = np.random.randint(0, 2, num_samples)
    tx_signal = 2 * bits - 1
    rx_signal_B1 = np.sqrt(P_A * alpha1) * np.sum(h_B1, axis=1) * tx_signal + np.sqrt(epsilon * P_A * alpha2) * np.sum(h_B1, axis=1) * tx_signal + np.sqrt(SNR_Eve_linear * d_E**alpha * N_0 / d_EB1**alpha) * np.sum(h_EB1, axis=1) * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
    rx_signal_B2 = np.sqrt(P_A * alpha2) * np.sum(h_B2, axis=1) * tx_signal + np.sqrt(SNR_Eve_linear * d_E**alpha * N_0 / d_EB2**alpha) * np.sum(h_EB2, axis=1) * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
    rx_signal_E = np.sqrt(P_A * alpha1) * np.sum(h_E, axis=1) * tx_signal + np.sqrt(P_A * alpha2) * np.sum(h_E, axis=1) * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
    rx_bits_B1 = (rx_signal_B1.real > 0).astype(int)
    rx_bits_B2 = (rx_signal_B2.real > 0).astype(int)
    rx_bits_E = (rx_signal_E.real > 0).astype(int)
...
plt.legend()
plt.grid(True)
plt.savefig('scenario_3_rs1_rs2.png')
plt.show()
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig('scenario_3_sop1_sop2.png')
plt.show()
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig('scenario_3_ip1_ip2.png')
plt.show()
plt.close()
...
plt.legend()
plt.grid(True, which='both')
plt.savefig('scenario_3_ber_b1_b2_e.png')
plt.show()
plt.close()

plt.figure(figsize=(8,6))
plt.plot(d_E_range, eta_s1_3, 'g-o', label='Secrecy Spectral Efficiency Bob1')
plt.plot(d_E_range, eta_s2_3, 'b-s', label='Secrecy Spectral Efficiency Bob2')
plt.xlabel('Khoảng cách d_E (m)')
plt.ylabel('Secrecy Spectral Efficiency (bits/s/Hz)')
plt.title('Secrecy Spectral Efficiency vs d_E')
plt.legend()
plt.grid(True)
plt.savefig('scenario_3_eta_s1_s2.png')
plt.show()
plt.close()