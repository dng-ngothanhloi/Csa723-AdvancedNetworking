...
epsilon = 0.01  # Tỷ lệ lỗi SIC (Cải tiến: SIC không hoàn hảo)
...
# Quét SNR_Bob (Cải tiến: Thêm quét SNR_Bob thay vì cố định)
...
for bob_idx, SNR_Bob_dB in enumerate(SNR_Bob_dB_range):
    ...
    for eve_idx, SNR_Eve_dB in enumerate(SNR_Eve_dB_range):
        ...
        # SNR (Cải tiến: Thêm lỗi SIC và sửa SNR_E)
        ...
        SNR_E1 = (P_A_eff * alpha1 * np.abs(h_E)**2) / (d_E**alpha * N_0 + P_A_eff * alpha2 * np.abs(h_E)**2)
        SNR_E2 = (P_A_eff * alpha2 * np.abs(h_E)**2) / (d_E**alpha * N_0)
        ...
        # BER (Cải tiến: Mô phỏng BPSK thực tế)
        bits = np.random.randint(0, 2, num_samples)
        tx_signal = 2 * bits - 1  # BPSK: +1 hoặc -1
        ...
        print(f"SNR_Bob = {SNR_Bob_dB} dB, SNR_Eve = {SNR_Eve_dB} dB | C_B1 = {C_B1[bob_idx, eve_idx]/1e6:.2f} Mbps | C_B2 = {C_B2[bob_idx, eve_idx]/1e6:.2f} Mbps | C_E = {C_E[bob_idx, eve_idx]/1e6:.2f} Mbps | BER_B1 = {BER_B1[bob_idx, eve_idx]:.4e} | BER_B2 = {BER_B2[bob_idx, eve_idx]:.4e} | BER_E = {BER_E[bob_idx, eve_idx]:.4e}")

# Lưu kết quả (Cải tiến: Lưu vào file .npy)
...
# Vẽ biểu đồ (chỉ cho SNR_Bob_dB = 10 dB để so sánh với mã gốc)
bob_idx_plot = np.where(SNR_Bob_dB_range == 10)[0][0]
...
plt.title('Secrecy Rate vs SNR_Eve (SNR_Bob = 10 dB)')
plt.legend()
plt.grid(True)
plt.show()
...
plt.title('Secrecy Outage Probability vs SNR_Eve (SNR_Bob = 10 dB)')
plt.legend()
plt.grid(True)
plt.show()
...
plt.title('Intercept Probability vs SNR_Eve (SNR_Bob = 10 dB)')
plt.legend()
plt.grid(True)
plt.show()
...
plt.title('BER vs SNR_Eve (SNR_Bob = 10 dB)')
plt.legend()
...
SNR_Bob_dB = 10  # Cố định SNR_Bob cho kịch bản 3
SNR_Bob_linear = 10**(SNR_Bob_dB / 10)
