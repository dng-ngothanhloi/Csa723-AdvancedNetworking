...
N_0 = 1e-15  # Nhiễu nền (W), tăng lên để phản ánh thực tế
...
R_th = 5.0  # Ngưỡng bảo mật (bits/s/Hz), tăng lên để phản ánh thực tế
epsilon = 0.01  # Tỷ lệ lỗi SIC, tăng lên để mô phỏng lỗi thực tế
...
for bob_idx, SNR_Bob_dB in enumerate(SNR_Bob_dB_range):
    ...
    for eve_idx, SNR_Eve_dB in enumerate(SNR_Eve_dB_range):
        ...
        I_Bob = P_A_eff * np.abs(h_E)**2 / (d_E**alpha)
...
SNR_Bob_dB = 20  # Cập nhật SNR_Bob
SNR_Eve_dB = 30  # Thêm SNR_Eve cố định
SNR_Bob_linear = 10**(SNR_Bob_dB / 10)
SNR_Eve_linear = 10**(SNR_Eve_dB / 10)
num_samples = int(1e5)  # Tăng số lần mô phỏng lên 10^5
...
for idx, d_E in enumerate(d_E_range):
    ...
    h_EB2 = np.sqrt(0.5) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
    d_EB1 = abs(d_E - d_B1) + 1e-3
    d_EB2 = max(abs(d_E - d_B2), 1)  # Điều chỉnh để tránh nhiễu khi d_E ≈ d_B2

    # SNR
    SNR_B2 = (P_A * alpha2 * np.abs(h_B2)**2) / (d_B2**alpha * N_0 + SNR_Eve_linear * d_E**alpha * N_0 * np.abs(h_EB2)**2 / (d_EB2**alpha))
    SNR_B1 = (P_A * alpha1 * np.abs(h_B1)**2) / (d_B1**alpha * N_0 + epsilon * P_A * alpha2 * np.abs(h_B1)**2 + SNR_Eve_linear * d_E**alpha * N_0 * np.abs(h_EB1)**2 / (d_EB1**alpha))
    I_Bob = P_A * np.abs(h_E)**2 / (d_E**alpha)
    SNR_E1 = (P_A * alpha1 * np.abs(h_E)**2) / (d_E**alpha * N_0 + I_Bob)
    SNR_E2 = (P_A * alpha2 * np.abs(h_E)**2) / (d_E**alpha * N_0 + P_A * np.abs(h_E)**2 / max(d_E**alpha, d_B2**alpha))
    ...
    bits = np.random.randint(0, 2, num_samples)
    tx_signal = 2 * bits - 1
    rx_signal_B1 = np.sqrt(P_A * alpha1) * h_B1 * tx_signal + np.sqrt(epsilon * P_A * alpha2) * h_B1 * tx_signal + np.sqrt(SNR_Eve_linear * d_E**alpha * N_0 / d_EB1**alpha) * h_EB1 * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
    rx_signal_B2 = np.sqrt(P_A * alpha2) * h_B2 * tx_signal + np.sqrt(SNR_Eve_linear * d_E**alpha * N_0 / d_EB2**alpha) * h_EB2 * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
    rx_signal_E = np.sqrt(P_A * alpha1) * h_E * tx_signal + np.sqrt(P_A * alpha2) * h_E * tx_signal + np.sqrt(N_0) * (np.random.randn(num_samples) + 1j * np.random.randn(num_samples))
    rx_bits_B1 = (rx_signal_B1.real > 0).astype(int)
    rx_bits_B2 = (rx_signal_B2.real > 0).astype(int)
    rx_bits_E = (rx_signal_E.real > 0).astype(int)
